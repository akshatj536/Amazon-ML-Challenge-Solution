...
# --- Configuration ---
PARAMS = {
    'RUN_NAME': 'OptionB-QWEN3-Finetune',
    'DATASET_FOLDER': 'dataset/',
    'MODEL_NAME': 'Qwen/Qwen3-Embedding-4B',
    'MODEL_SAVE_PATH': 'best_model_qwen3',
    'BATCH_SIZE': 64,
    'LEARNING_RATE': 2e-5,
    'NUM_EPOCHS': 15,
    'DATA_PERCENTAGE': 1.0,
...
def main():
    with mlflow.start_run(run_name=PARAMS['RUN_NAME']):
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        tokenizer = AutoTokenizer.from_pretrained(PARAMS['MODEL_NAME'], trust_remote_code=True)
        model = AutoModel.from_pretrained(PARAMS['MODEL_SAVE_PATH'])
        regressor = Regressor(model.config.hidden_size, PARAMS['regressor_width'], PARAMS['regressor_depth'], PARAMS['DROPOUT_RATE'])
...
def run_inference(model, tokenizer, params, device):
    ...
    class TestDataset(Dataset):
        ...
        def __getitem__(self, idx):
            row = self.dataframe.iloc[idx]
            inputs = self.tokenizer(row['catalog_content'], return_tensors="pt", padding='max_length', truncation=True, max_length=512)
    ...
    submission_filename = f"submission_qwen3_{timestamp}.csv"
